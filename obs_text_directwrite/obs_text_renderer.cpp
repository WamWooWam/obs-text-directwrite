...
obs_text_renderer::obs_text_renderer(IDWriteFactory4 *pDWriteFactory_,
...
{
	{
		...
		if (pOutlineBrush) {
			...
			pD2DFactory->CreatePathGeometry(
				pGeometry.GetAddressOf());
			...
			D2D1::Matrix3x2F const matrix = D2D1::Matrix3x2F(
				1.0f, 0.0f, 0.0f, 1.0f, baselineOriginX,
				baselineOriginY);

			pD2DFactory->CreateTransformedGeometry(
				pGeometry.Get(), &matrix,
		...
		hr = pDWriteFactory->TranslateColorGlyphRun(
			baselineOrigin, glyphRun, glyphRunDescription,
...
IFACEMETHODIMP
obs_text_renderer::DrawUnderline(__maybenull void *clientDrawingContext,
...
IFACEMETHODIMP obs_text_renderer::DrawStrikethrough(
	__maybenull void *clientDrawingContext, FLOAT baselineOriginX,
...
IFACEMETHODIMP obs_text_renderer::DrawInlineObject(
...
IFACEMETHODIMP
obs_text_renderer::GetCurrentTransform(__maybenull void *clientDrawingContext,
				       __out DWRITE_MATRIX *transform)
{
	//forward the render target's transform
	pDeviceContext->GetTransform(
		reinterpret_cast<D2D1_MATRIX_3X2_F *>(transform));
	return S_OK;
}

IFACEMETHODIMP
obs_text_renderer::GetPixelsPerDip(__maybenull void *clientDrawingContext,
...
IFACEMETHODIMP obs_text_renderer::QueryInterface(IID const &riid,
						 void **ppvObject)
{
